    ...
    <nav class="border-b bg-white/80 backdrop-blur-md sticky top-0 z-50">
      <div class="container mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div class="flex items-center gap-10">
          <nuxt-link to="/"
            class="text-2xl font-black tracking-tighter uppercase flex items-center gap-2 text-orange-600">
            MoonLight <span
              class="text-[10px] bg-orange-600 text-white px-2 py-1 rounded-full not-italic tracking-normal">KITCHEN</span>
          </nuxt-link>
        ...
        <div class="flex items-center gap-5">
          ...
          <div v-if="user" class="relative z-50">
            <button @click="isMenuOpen = !isMenuOpen"
              ...
              <div
                class="w-8 h-8 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-black uppercase">
                {{ user.email[0] }}
              </div>
            ...
            <div v-show="isMenuOpen"
              ...
              <nuxt-link v-if="user.email === 'admin@gmail.com'" to="/admin" @click.native="isMenuOpen = false"
              ...
              <nuxt-link to="/orders" @click.native="isMenuOpen = false"
                class="block px-5 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition flex items-center gap-2">
                📦 ประวัติการสั่งซื้อ
              </nuxt-link>

              <button @click="handleLogout"
                class="w-full text-left px-5 py-2.5 text-sm text-red-500 hover:bg-red-50 transition flex items-center gap-2 font-bold mt-2 border-t border-gray-50">
                🚪 ออกจากระบบ
              </button>
            </div>
          </div>
          ...
          <nuxt-link v-else to="/login"
            class="text-xs font-black bg-black text-white px-6 py-2.5 rounded-full hover:bg-orange-600 transition shadow-lg uppercase tracking-widest">
            Login
          </nuxt-link>

          <nuxt-link to="/cart"
            class="relative bg-white p-2.5 rounded-full border border-gray-100 hover:shadow-md transition group">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-700 group-hover:text-orange-600"
      ...
      <div class="container mx-auto px-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          <div class="md:col-span-2">
            <h2 class="text-3xl font-black tracking-tighter uppercase mb-6 text-orange-600">MoonLight KITCHEN</h2>
            ...
            <div class="flex gap-4 mt-6">
              <div
                class="w-8 h-8 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 hover:text-orange-600 transition cursor-pointer">
                FB</div>
              <div
                class="w-8 h-8 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 hover:text-orange-600 transition cursor-pointer">
                IG</div>
        ...
        <div
          class="border-t border-gray-50 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-gray-400 text-[10px] uppercase font-bold tracking-widest">
          <p>&copy; 2026 MOONLIGHT KITCHEN. ALL RIGHTS RESERVED.</p>
...
export default {
  ...
  computed: {
    cartTotal() {
      // ดึงค่าจาก Store อย่างปลอดภัย
      return this.$store.getters['cart/totalItems'] || 0
    }
  },
 async mounted() {
  // 1. ดึง session ทันทีที่เข้าหน้าเว็บ
  const { data: { session } } = await this.$supabase.auth.getSession()
  
  if (session) {
    this.user = session.user
    // สำคัญ: ต้องรีบดึงของในตะกร้ากลับมาทันทีที่ refresh
    await this.$store.dispatch('cart/fetchCart')
  }

  // 2. เฝ้าติดตามการเปลี่ยนแปลง (เผื่อเปิดหลาย tab หรือ session หมดอายุ)
